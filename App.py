    ...
    try:
        ...
        for key, filename in model_files.items():
            ...
            if os.path.exists(model_path):
                models[key] = pickle.load(open(model_path, 'rb'))
        return models
    except Exception as e:
        st.error("Models not found. Place .sav files in 'model/' folder.")
        return {}
...
class PredictionManager:
    ...
    def predict(self, model_name, user_input):
        ...
        try:
            ...
            prob = getattr(model, 'predict_proba', lambda x: [0.8, 0.2])([user_input])[0]
            confidence = max(prob)
            
            risk_map = {
                'diabetes': 'Moderate Risk' if prediction else 'Low Risk',
...
def load_css():
    ...
    .metric-card {background: linear-gradient(90deg, #f8f9fa, #e9ecef); 
                  border-radius: 12px; padding: 1.5rem; margin: 0.5rem 0;}
    .risk-high {background: linear-gradient(90deg, #f44336, #d32f2f); 
                color: white; padding: 0.5rem 1rem; border-radius: 20px;}
    .risk-moderate {background: linear-gradient(90deg, #FF9800, #f57c00); 
                    color: white; padding: 0.5rem 1rem; border-radius: 20px;}
    .risk-low {background: linear-gradient(90deg, #4CAF50, #45a049); 
               color: white; padding: 0.5rem 1rem; border-radius: 20px;}
    .stButton > button {background: linear-gradient(90deg, #667eea 0%, #764ba2 100%); 
                        border-radius: 10px; border: none; padding: 0.75rem 2rem;}
    </style>
    """, unsafe_allow_html=True)

def auth_sidebar():
    if 'authenticated' not in st.session_state:
        st.session_state.authenticated = not users_collection  # Skip if no DB
...
def render_dashboard(models):
    st.markdown('<div class="header"><h1>🏥 Healthcare Dashboard</h1></div>', unsafe_allow_html=True)
    
    col1, col2, col3, col4 = st.columns(4)
    with col1: st.metric("Tests Completed", 12, delta="+2")
    with col2: st.metric("Diseases Tested", 3)
    with col3: st.metric("Last Test", "Today")
    with col4: st.metric("High Risk", 2, delta="+1")
    
    col1, col2 = st.columns(2)
    with col1:
        st.subheader("🚀 Quick Tests")
        if st.button("🩺 Diabetes", key="diabetes_btn"): st.session_state.page = "diabetes"
        if st.button("❤️ Heart", key="heart_btn"): st.session_state.page = "heart" 
        if st.button("🧠 Parkinson", key="parkinson_btn"): st.session_state.page = "parkinsons"
...
def render_prediction_page(disease, feature_config, models):
    st.markdown(f'<div class="header"><h1>{disease.replace("_", " ").title()}</h1></div>', unsafe_allow_html=True)
    
    # Dynamic inputs based on disease
    inputs = {}
    for i, (name, minv, maxv, default) in enumerate(feature_config):
        col_idx = i % 4
        if col_idx == 0: st.columns(4)
        inputs[name] = st.number_input(name, min_value=minv, max_value=maxv, value=default)
    
    if st.button("🔬 Predict Risk", use_container_width=True):
        user_input = list(inputs.values())
        result = PredictionManager(models).predict(disease, user_input)
        ...
        if result:
            ...
            with col1:
                fig = go.Figure(go.Indicator(
                    ...
                    title={"text": f"{result.risk_level}"},
                    gauge={'bar': {'color': "#4CAF50" if "low" in result.risk_level.lower() else "#FF9800"}}))
                st.plotly_chart(fig)
            ...
            with col2:
                ...
                <div class="metric-card">
                    ...
                    <span class="risk-{result.risk_level.split()[0].lower()}">{result.risk_level}</span>
                </div>
                """, unsafe_allow_html=True)
        else:
...
if not st.session_state.authenticated:
    ...
    models = load_models()
    if not models:
        st.warning("⚠️ No models loaded. Create 'model/' folder with .sav files.")
    ...
    with st.sidebar:
        page = option_menu("Menu", 
            ['Dashboard', 'Diabetes', 'Heart Disease', 'Parkinson', 'History', 'Export'],
            icons=['speedometer', 'activity', 'heart', 'person', 'clock', 'download'],
            default_index=0)[file:1]
    ...
    if page == 'Dashboard':
        ...
        diabetes_features = [
            ...
            ('Skin Thickness', 0, 99, 20),
            ('Insulin', 0, 846, 79),
            ('BMI', 0.0, 67.1, 32.0),
            ('Pedigree', 0.0, 2.42, 0.47),
            ('Age', 0, 120, 30)
        ]
        render_prediction_page('diabetes', diabetes_features, models)
    elif page == 'Heart Disease':
        heart_features = [
            ('Age', 1, 120, 52), ('Sex', 0, 1, 1),
            ('Chest Pain', 0, 3, 2), ('Rest BP', 80, 200, 120),
            ('Cholesterol', 100, 600, 230)
        ]
        render_prediction_page('heart_disease', heart_features, models)
    elif page == 'Parkinson':
        st.info("Parkinson's analysis - 23 voice features (simplified demo)")
    elif page == 'History':
        st.dataframe(pd.DataFrame({"Date": ["2025-12-29"], "Disease": ["Diabetes"], 
                                 "Result": ["Negative"], "Risk": ["Low"]}))
    elif page == 'Export':
        st.download_button("Download CSV", "data", "health_data.csv")
